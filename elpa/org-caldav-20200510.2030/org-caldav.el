...
(defun org-caldav-update-events-in-org ()
  ...
  (let (
	(events (append (org-caldav-filter-events 'new-in-cal) (org-caldav-filter-events 'changed-in-cal)))
	(url-show-status nil)
	(counter 0)
	 ...
	 (t
	  ...
	  (let ((marker (org-id-find (car cur) t)))
		...
		(org-caldav-change-location (nth 6 eventdata)))
	      (when (or (message "chekaem") (eq org-caldav-sync-changes-to-org 'timestamp-only)
			(eq org-caldav-sync-changes-to-org 'title-and-timestamp))
		;; Sync timestamp
		(setq timesync
